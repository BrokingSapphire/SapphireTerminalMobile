...
class ClosedOptionGridScreen extends StatefulWidget {
  const ClosedOptionGridScreen({super.key});

  @override
  State<ClosedOptionGridScreen> createState() => _ClosedOptionGridScreen();
}

class _ClosedOptionGridScreen extends State<ClosedOptionGridScreen> {
    ...
    if (_sortColumn != null) {
      dataList.sort((a, b) {
        ...
        if (['entry', 'exit', 'netgain', 'margin'].contains(_sortColumn)) {
          double aNum = double.tryParse(aValue.replaceAll(RegExp(r'[₹,]'), '')) ?? 0;
          double bNum = double.tryParse(bValue.replaceAll(RegExp(r'[₹,]'), '')) ?? 0;
          return _isAscending ? aNum.compareTo(bNum) : bNum.compareTo(aNum);
        }
        // Handle duration (numeric, convert to int)
        else if (_sortColumn == 'duration') {
